...
import { OnChangeReturnsInput, RemoveReturnItem, SetReturnItemList } from '../../redux/state_slice/returnslice';
...
const ReturnCreateUpdate = () => {
  ...
  const CustomerDropDown = useSelector(
    (state: RootState) => state.sale.CustomerDropDown,
  );
  const ProductDropDown = useSelector(
    (state: RootState) => state.sale.ProductDropDown,
  );
  const ParentData = useSelector(
    (state: RootState) => state.return.ReturnFormValue,
  );
  const ChildData = useSelector((state: RootState) => state.return.ReturnItemList);
  ...
  const CreateNewSale = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    let res = await CreateReturnRequest(ParentData, ChildData);
    if (res) {
      navigate('/retrun-list');
    }
  };
    ...
    <section>
      <div className="md:grid grid-cols-12 grid-rows-12 w-full gap-4 space-y-4">
        <div className="bg-white shadow-xl col-span-5 row-span-12 rounded-md px-4 space-y-4  py-8">
          <h1 className="font-bold text-2xl text-blue-400">Create Return</h1>
          {/* Parent Input  */}
